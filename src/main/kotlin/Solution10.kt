class Solution10 {
    fun isMatch(s: String, p: String): Boolean {
        ...
        return dfs(s, p, 0, 0, m, n, matrix) // The problem: to match s[i, m), p[j, n)
    }

    // the sub-problem: to match[i, m
    private fun dfs(s: String, p: String, i: Int, j: Int, m: Int, n: Int, matrix: Array<Array<Boolean?>>): Boolean {
        if (j == n) return i == m
        if (matrix[i][j] != null) return matrix[i][j]!!
        val c2 = p[j]
        return if (j < n - 1 && p[j + 1] == '*')
            dfs(s, p, i, j + 2, m, n, matrix) ||  // do not match 'x*' , x means any char or . (use'*' as 0 char)
                    (i < m && match(s[i], c2) && dfs(s, p, i + 1, j, m, n, matrix)
                        .also {
                            matrix[i][j] = it // match 1 char in string for '*'
                        }
                            )
        else i < m && match(s[i], c2) && dfs(s, p, i + 1, j + 1, m, n, matrix)
            .also {
                matrix[i][j] = it // match 1 char from both sides
            }
    }
    ...
    fun isMatch2(s: String, p: String): Boolean {
        val m = s.length
        val n = p.length
        val matrix = Array(m + 1) { BooleanArray(n + 1) }
        for (i in m downTo 0) {
            for (j in n downTo 0) {
                if (j == n) {
                    matrix[i][j] = i == m
                } else if (j + 1 < n && p[j + 1] == '*') {
                    matrix[i][j] = matrix[i][j + 2] || (i < m && match(s[i], p[j]) && matrix[i + 1][j])
                } else { // j == cols - 1 || j < cols - 1 && p.charAt(j + 1) != '*'
                    matrix[i][j] = i < m && match(s[i], p[j]) && matrix[i + 1][j + 1]
                }
            }
        }
        return matrix[0][0]
